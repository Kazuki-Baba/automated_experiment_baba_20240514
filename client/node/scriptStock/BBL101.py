...
Run = [0x02, 0x30, 0x31, 0x57, 0x52, 0x55, 0x4E,
       0x30, 0x30, 0x30, 0x30, 0x31, 0x03]  # 実行
Stop = [0x02, 0x30, 0x31, 0x57, 0x52, 0x55, 0x4E,
        0x30, 0x30, 0x30, 0x30, 0x30, 0x03]  # 停止
SettingExample = [0x02, 0x30, 0x31, 0x57, 0x53,
                  0x56, 0x31, 0x30, 0x30, 0x33, 0x37, 0x30, 0x03]
...
def tempGet():
    ...
    if isPv1 == 'PV1':
        ...
        temperature = 0.
        temperature = 100. * float(temperatureRaw[0]) + 10. * float(
            temperatureRaw[1]) + float(temperatureRaw[2]) + 0.1 * float(temperatureRaw[3])
        return temperature
    return None
...
def start():
    cmd = [0x02, 0x30, 0x31, 0x57, 0x52, 0x55, 0x4E,
           0x30, 0x30, 0x30, 0x30, 0x31, 0x03]  # 実行
    BCC_function(cmd)
    commandInput(cmd)
...
errorMsg1 = ["センサ異常","ヒーター短絡異常","ヒーター断熱異常","独立過昇防止異常","冷凍機異常"]
errorMsg2 = ["メモリ異常","AT異常","内部通信異常","空焚き異常","ドアオープン"]

if args.status:
    cmd = [0x02, 0x30, 0x31, 0x52, 0x45, 0x52, 0x31, 0x03] #エラーメッセージ1
    cmd2 = [0x02, 0x30, 0x31, 0x52, 0x45, 0x52, 0x32, 0x03] #エラーメッセージ2
    BCC_function(cmd)
    commandInput(cmd)
    res = commandReception(ser)
    resNumber = res[7:12]
    checkStatus(resNumber,errorMsg1)
    ...
    res = commandReception(ser)
    resNumber = res[7:12]
    checkStatus(resNumber,errorMsg2)
