...
client.on("connection", (socket) => {
  ...
  // baba_modified_20240829
  socket.on("experimentResUpdate", function(a) {
    //console.log(a);
    procedureNumber = a.procedure.procedureOrder;
    experimentRecipe[procedureNumber - 1].result = a.procedure.result;
    srvUser.emit("experimentRecipe", experimentRecipe);
    
    if (a.procedure.is_result_flag == 1) {
      let getDate = createDate(); //20240319_baba
      con.query(
            "SELECT id FROM result_procedure_blocks WHERE result_procedure_id = ? AND result_block_order = ?",
            [a.procedure.resultProcedureId, a.procedure.procedureOrder],
            (e, r, f) => {
              let resultBlockId = r[0].id;
              con.query(
                "UPDATE result_data SET data=? , created_at=? WHERE result_block_id=?",
                [a.procedure.result, getDate, resultBlockId], //modified_0829
                (er, rr, fi) => {
                  if (er) {
                    console.log(er);
                }
...
function emitScriptByDeviceType(device,emitTarget){
  ...
  Object.keys(device).forEach(function (key){
    ...
    if((deviceObj.device_type_name == "thermometer") || (deviceObj.device_type_name == "temperature controller") || (deviceObj.device_type_name == "Digital Temp Controller")){ // 20240607_baba_added
